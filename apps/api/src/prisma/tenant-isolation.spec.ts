...
describe('Multi-Tenant Isolation Integration Gate', () => {
  ...
  beforeAll(async () => {
    ...
    // Clean up existing test records to prevent unique key collisions
    ...
    // 2. Seed mock records for each tenant
    // Tenant A Project, Sprint & Task
  ...
  it('should isolate queries on Project and Task between Tenant A and Tenant B', async () => {
    ...
    await tenantLocalStorage.run({ tenantId: tenantAId }, async () => {
      ...
      expect(projects[0].name).toBe('Project Tenant A');
      expect(projects[0].tenant_id).toBe(tenantAId);

      expect(tasks.length).toBe(1);
      expect(tasks[0].title).toBe('Task Tenant A');
      expect(tasks[0].tenant_id).toBe(tenantAId);
    });
    ...
    await tenantLocalStorage.run({ tenantId: tenantBId }, async () => {
      ...
      // Should only see Tenant B records
      expect(projects.length).toBe(1);
      expect(projects[0].name).toBe('Project Tenant B');
      expect(projects[0].tenant_id).toBe(tenantBId);

      expect(tasks.length).toBe(1);
      expect(tasks[0].title).toBe('Task Tenant B');
      expect(tasks[0].tenant_id).toBe(tenantBId);
    });
  });
});
