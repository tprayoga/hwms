...
import { CheckinType, TaskStatus } from '@prisma/client';
...
export class DashboardService {
  ...
  async getTeamDashboard(userId: string, teamFilter: string, dateFromStr: string, dateToStr: string) {
    ...
    for (const c of checkins) {
      ...
      if (c.type === CheckinType.IN) {
        ...
        grouped[groupKey].standupTasks = c.standup_items.map((item: any) => ({
          id: item.task.id,
          code: item.task.code,
    ...
    const attendanceList = Object.values(grouped).map(g => {
      ...
      if (g.checkout) {
        const doneTasks = g.standupTasks.filter((t: any) => t.statusAfter === TaskStatus.DONE);
        if (doneTasks.length > 0) {
          // Simplistic evidence check: does it have evidence uploaded
          noEvidence = false; 
        }
      }
    ...
    const anomaliesCount = attendanceList.filter(a => 
      a.flags.late || a.flags.auto || a.flags.offline || !a.flags.geofence_ok
    ).length;
  ...
  async getProgramDashboard() {
    ...
    const sprintMetrics = sprints.map(s => {
      ...
      let rag = 'RED';
      if (progress >= 85) rag = 'GREEN';
      else if (progress >= 60) rag = 'YELLOW';
    ...
    const roleCompletion: { [key: string]: { name: string, totalWeight: number, weightedProgress: number } } = {};
    if (activeSprint) {
