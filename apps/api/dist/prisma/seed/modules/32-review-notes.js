...
exports.seedReviewNotes = seedReviewNotes;
const upsert_1 = require("../lib/upsert");
const WEEKS = 4;
const HIGHLIGHT_TEMPLATES = [
    (a, b) => `${a} dan ${b} menuntaskan mayoritas task sprint tepat waktu.`,
    (a, b) => `Progres solid dari ${a}; ${b} perlu dukungan pada item ber-risiko.`,
    (a, b) => `${a} disiplin check-in; ${b} menutup satu blocker penting.`,
    (a, b) => `Kolaborasi ${a} & ${b} membaik minggu ini, throughput naik.`,
];
...
async function seedReviewNotes(prisma, ctx) {
    const log = (0, upsert_1.createLogger)('32-review-notes');
    const rng = ctx.rng;
    ...
    for (let i = WEEKS; i >= 1; i--)
        weeks.push(new Date(ctx.anchor.getTime() - i * 7 * 24 * 3600 * 1000));
    ...
    for (const mgr of managers) {
        ...
        for (const week of weeks) {
            ...
            await (0, upsert_1.findOrCreate)(prisma.reviewNote, { tenant_id: tenantId, author_id: mgr.id, week_start: week }, {
                tenant_id: tenantId,
                author_id: mgr.id,
                week_start: week,
                highlights,
...
//# sourceMappingURL=32-review-notes.js.map